...
/** LocalBusiness JSON-LD for contact / citations (same NAP as footer). */
function localBusinessJsonLd(baseUrl) {
  return {
    '@context': 'https://schema.org',
    ...
    description: 'Educational fatigue management, HOS-aligned rest tools, training packets, and wellness resources for professional truck drivers.',
    email: 'joyce@mile12warrior.com',
    telephone: '+1-916-292-7411',
    address: {
      '@type': 'PostalAddress',
...
/**
 * FAQPage JSON-LD from [{ question, answer }] (plain text answers preferred).
 */
function faqPageJsonLd(faqs) {
  const entities = (faqs || []).filter(function (f) {
...
const STATIC_SITEMAP_PATHS = [
  ...
  { path: '/packets/fleet-new-hire', changefreq: 'monthly', priority: '0.7' },
  { path: '/packets/fleet-refresher', changefreq: 'monthly', priority: '0.7' },
