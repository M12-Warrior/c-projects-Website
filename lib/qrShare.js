...
function withUtm(path, source, medium, campaign) {
...
function presetUrl(presetId, req) {
  ...
  return siteBaseUrl(req) + withUtm(preset.path, 'qr', 'offline', presetId);
}
...
function getShareLinks(req) {
  ...
  return Object.keys(SHARE_PRESETS).map(function (id) {
    ...
    const url = base + withUtm(preset.path, 'qr', 'offline', id);
    return {
      id,
      label: preset.label,
      description: preset.adminDescription,
      url,
      /** Suggested link for posting on X — attributed in Traffic as source "x" */
      xUrl: base + withUtm(preset.path, 'x', 'social', id),
